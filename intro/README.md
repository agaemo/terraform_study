...
| # | IaCを使わない場合の問題 | IaCによる解決 | キーワード |
|---|----------------------|-------------|-----------|
| 1 | **変更履歴が残らない**<br>GUI操作は誰がいつ何を変えたか記録が残らない。障害時のロールバックも困難。 | 設定ファイルを Git で管理することで、変更履歴・レビュー・差し戻しがコードと同じワークフローで行える。 | 追跡可能性 |
| 2 | **環境を再現できない**<br>手順書・口伝では本番/stg/dev で設定がずれる「環境ドリフト」が起きる。 | コードが環境の定義そのものになるため、同じファイルを適用すれば何度でも同じ環境を再現できる。 | 冪等性 |
| 3 | **チームで設定が統一されない**<br>メンバーごとに手順が異なり設定がばらつく。引き継ぎも属人的になる。 | コードがインフラの仕様書を兼ね、レビューで合意形成できる。新メンバーもコードを読めば構成を把握できる。 | チーム協業 |
| 4 | **依存関係の管理が複雑**<br>リソースの作成順序を手動で把握・管理しなければならない。 | Terraform がリソースグラフを自動解析し、依存関係を正しい順序・並列で処理する。 | 自動化 |
| 5 | **同じ構成を環境ごとに書き直す**<br>dev/stg/prod で設定が重複し、変更のたびに全環境を修正する必要がある。 | モジュールで構成をテンプレート化し、変数を切り替えるだけで同じ構成を複数環境に展開できる。 | 標準化 |
| 6 | **クラウドごとにツールが異なる**<br>AWS・GCP・Azure それぞれ専用のツールや知識が必要になる。 | Terraform の統一ワークフローで複数クラウドをまとめて管理できる。 | マルチクラウド |

**具体的には以下のような場面で使われる：**

- **マルチクラウド管理** — AWS・Azure・GCP を同一ワークフローで扱い、ベンダーロックインを避ける
- **並列環境の管理** — dev / stg / prod を同じ `.tf` から変数だけ切り替えて展開。不要環境は `destroy` で即削除
- **Kubernetes 管理** — クラスタのプロビジョニング（EKS・GKE）から Pod・Service までを同一ツールで管理
- **多層構成のデプロイ** — Web サーバー・DB・LB など依存関係のあるリソースを一括定義・並列デプロイ
...
Terraform の設定ファイルは **HCL（HashiCorp Configuration Language）** で書く。
`resource`・`variable`・`locals` などは HCL の**予約済みブロックタイプ**で、Terraform が意味を定義している。自分では作れない。
...
| ブロックタイプ | 役割 | ラベル |
|-------------|------|--------|
| `terraform` | プロバイダーのバージョン宣言など | なし |
| `provider` | プロバイダーの設定（リージョン・認証） | プロバイダー名 |
| `resource` | 作成・管理するリソースの定義 | リソースタイプ・ローカル名 |
| `variable` | 外から渡せる入力変数の定義 | 変数名 |
| `locals` | 内部で使う中間値の定義 | なし（`locals` の中にキーを書く） |
| `output` | apply 後に表示・公開する値の定義 | 出力名 |
| `data` | 既存リソースの参照（作成・削除しない） | データソースタイプ・ローカル名 |
| `module` | 別ディレクトリのモジュールを呼び出す | モジュール名 |

`locals` だけ複数形なのは、1つのブロックに複数のキーをまとめて書く設計のため。
