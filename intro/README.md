...
> **IaC の回答**
> 設定ファイルを Git で管理することで、変更履歴・レビュー・差し戻しがコードと同じワークフローで行える。
...
> **IaC の回答**
> コードが環境の定義そのものになるため、同じファイルを適用すれば何度でも同じ環境を再現できる（冪等性）。
...
> **IaC の回答**
> コードがインフラの仕様書を兼ねるため、レビューを通じてチームの合意が得られ、新メンバーもコードを読めば構成を把握できる。
