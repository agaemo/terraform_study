...
| # | IaCを使わない場合の問題 | IaCによる解決 |
|---|----------------------|-------------|
| 1 | **変更履歴が残らない**<br>GUI操作は誰がいつ何を変えたか記録が残らない。障害時のロールバックも困難。 | 設定ファイルを Git で管理することで、変更履歴・レビュー・差し戻しがコードと同じワークフローで行える。 |
| 2 | **環境を再現できない**<br>手順書・口伝では本番/stg/dev で設定がずれる「環境ドリフト」が起きる。 | コードが環境の定義そのものになるため、同じファイルを適用すれば何度でも同じ環境を再現できる（冪等性）。 |
| 3 | **チームで設定が統一されない**<br>メンバーごとに手順が異なり設定がばらつく。引き継ぎも属人的になる。 | コードがインフラの仕様書を兼ね、レビューで合意形成できる。新メンバーもコードを読めば構成を把握できる。 |
| 4 | **依存関係の管理が複雑**<br>リソースの作成順序を手動で把握・管理しなければならない。 | Terraform がリソースグラフを自動解析し、依存関係を正しい順序・並列で処理する。 |
| 5 | **同じ構成を環境ごとに書き直す**<br>dev/stg/prod で設定が重複し、変更のたびに全環境を修正する必要がある。 | モジュールで構成をテンプレート化し、変数を切り替えるだけで同じ構成を複数環境に展開できる。 |
