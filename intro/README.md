...
GUI（マネジメントコンソールなど）で手動操作すると、誰がいつ何を変えたか記録が残らない。
障害が起きても原因の変更を特定できず、ロールバックも困難になる。

**IaC の回答：** 設定ファイルを Git で管理することで、変更履歴・レビュー・差し戻しがコードと同じワークフローで行える。

### 問題2：環境を再現できない

手順書や口伝でインフラを構築すると、本番・ステージング・開発で微妙に設定が異なる「環境ドリフト」が起きる。
「自分の環境では動く」問題の多くはここに起因する。

**IaC の回答：** コードが環境の定義そのものになるため、同じファイルを適用すれば何度でも同じ環境を再現できる（冪等性）。

### 問題3：チームで設定が統一されない

メンバーごとに手順が異なると、設定のばらつきが蓄積する。
新メンバーへの引き継ぎも属人的な手順書に依存してしまう。

**IaC の回答：** コードがインフラの仕様書を兼ねるため、レビューを通じてチームの合意が得られ、新メンバーもコードを読めば構成を把握できる。
